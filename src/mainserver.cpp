...
// ============================================================
// 1. HÀM MAIN PAGE (Giao diện HTML)
// ============================================================
String mainPage()
{
    // Lấy dữ liệu Cảm biến (chỉ Peek để xem, không lấy đi)
   SensorData_t receivedData; 
    float temperature = 0.0;
    float humidity = 0.0;
    if (xQueuePeek(xServerQueue, &receivedData, 0) == pdTRUE) {
    ...
    AIResult_t aiData;
    float ai_score = 0.0; 
    if (xQueuePeek(xAIResultQueue, &aiData, 0) == pdTRUE) {
  ...
  <body>
    ...
    <div class="container">
      ...
      <div class="sensor ai-display">
        🧠 AI Score: <span id="ai" class="ai-val">)rawliteral" + String(ai_score, 3) + R"rawliteral(</span>
      </div>
      <div>
        <button onclick='toggleLED(1)'>💡 LED_Blinkdy: <span id="l1">)rawliteral" +
         led1 + R"rawliteral(</span></button>
        <button onclick='toggleLED(2)'>💡 LED_Neo: <span id="l2">)rawliteral" +
...
void handleToggle()
{
  ...
  if (led == 1)
  {
   ...
   Serial.println(led1_state ? "LED1/led_blinky ON" : "LED1/led_blinky OFF");
  }
  else if (led == 2)
  {
    led2_state = !led2_state;
    Serial.println(led2_state ? "LED2/neo_blinky ON" : "LED2/neo_blinky OFF");
...
void handleSensors(){
    ...
    // "Nhìn" (Peek) vào hàng đợi để lấy dữ liệu mới nhất

    // mà không xóa nó khỏi hàng đợi.

    if (xQueueReceive(xServerQueue, &receivedData, 0) == pdTRUE) {
    ...
    AIResult_t aiData;
    static float ai_score = 0.0; // Dùng static để giữ giá trị cũ nếu queue rỗng
    ...
    String json = "{";
    json += "\"temp\":" + String(temperature) + ",";  // Có dấu phẩy
    json += "\"hum\":" + String(humidity) + ",";      // <-- QUAN TRỌNG: Thêm dấu phẩy ở đây
    json += "\"ai\":\"" + String(ai_score, 3) + "\""; // AI Score
    json += "}";
    server.send(200, "application/json", json);
...
xAIResultQueue = xQueueCreate(1, sizeof(AIResult_t));
  ...
  if(xAIResultQueue == NULL){
      Serial.println("Lỗi: Không tạo được Queue AI!");
  }
  startAP();
  setupServer();
