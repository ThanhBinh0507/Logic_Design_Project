...
//   float humidity = glob_humidity;
  SensorData_t receivedData; // Tạo một biến cục bộ
    float temperature = 0.0;   // Giá trị mặc định nếu queue rỗng
    float humidity = 0.0;      // Giá trị mặc định nếu queue rỗng

    // "Nhìn" (Peek) vào hàng đợi để lấy giá trị ban đầu
